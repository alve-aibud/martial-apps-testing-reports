...
- Name: specs/smoke-testing/14-child-profile/parent-child.screen-time-extension.spec.ts >> Screen-time extension round trip >> PAR-008 - child requests extra time, parent denies it @PAR-008 @destructive
- Location: e2e/specs/smoke-testing/14-child-profile/parent-child.screen-time-extension.spec.ts:206:7

# Error details

```
Error: no pending screen-time request found for "Alnf Child" at 53 minutes.

expect(locator).toBeVisible() failed

Locator: locator('div').filter({ has: getByText('Alnf Child') }).filter({ has: getByText(/\b53\s+minutes\b/) }).first()
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - no pending screen-time request found for "Alnf Child" at 53 minutes. with timeout 5000ms
  - waiting for locator('div').filter({ has: getByText('Alnf Child') }).filter({ has: getByText(/\b53\s+minutes\b/) }).first()
...
Diagnostic dump at failure time:
  tabs: Pending2 | Approved32 | Denied23
    ...
    - generic [ref=f4e12]:
      - generic [ref=f4e13]:
        - button "Pending 2" [active] [ref=f4e14] [cursor=pointer]:
          - text: Pending
          - generic [ref=f4e18]: "2"
        - button "Approved 32" [ref=f4e19] [cursor=pointer]:
          - text: Approved
          - generic [ref=f4e23]: "32"
...
> 329 |       throw new Error(`${(err as Error).message}\n\n${await this.dumpRequestsModalState()}`);
  ...
  369 | 
  370 |   /**
  371 |    * `PAR-007` step 5's own confirmation: the request moved to the Approved
  372 |    * tab. Must switch tabs first - the modal's list is server-filtered on
  373 |    * `status: activeTab`, so the just-approved request has already vanished
  374 |    * from whichever tab was open when the approval landed (normally Pending).
  375 |    *
  376 |    * The whole click-tab / widen-date / find-card sequence is wrapped in
  377 |    * `toPass` (not a single attempt) - found flaky live under a full
  378 |    * `test:smoke-full:dev` run 2026-08-31, AFTER the date-widening fix above
  379 |    * already existed, so the date boundary was not the whole story.
  380 |    * `ScreenTimeRequestCard.tsx`'s approve handler waits a full second
  381 |    * (`setTimeout`) after the API call succeeds before triggering the modal's
  382 |    * OWN refetch, and this method's tab click and date-widen EACH trigger
  383 |    * their own independent refetch on top of that - under real load (this
  384 |    * suite's full 17-group run, not an isolated file) it is plausible for
  385 |    * those to land out of order and the list to miss the item on the first
  386 |    * look. Retrying the whole sequence, not just the final assertion, means a
  387 |    * losing race self-heals on the next pass rather than needing Playwright's
  388 |    * OWN test-level retry to paper over it - the same reasoning
  389 |    * `profile-switch.page.ts`'s `expectInChildMode`/`expectInParentMode`
  390 |    * already use for a same-shaped click-then-look problem.
  391 |    */
  392 |   async expectRequestApproved(childName: string, requestedMinutes: number): Promise<void> {
  393 |     try {
  394 |       await expect(async () => {
  395 |         await screenTimeLocators.approvedTab(this.page).click();
  396 |         await this.widenEndDateFilter();
  397 |         const card = screenTimeLocators.requestCard(this.page, childName, requestedMinutes);
  398 |         await expect(
  399 |           screenTimeLocators.approvedStatusBadge(card),
  400 |           `the request for "${childName}" at ${requestedMinutes} minutes did not ` +
  401 |             'show up on the Approved tab after a successful approval.',
  402 |         ).toBeVisible({ timeout: 5_000 });
  403 |       }).toPass({ timeout: RENDER_TIMEOUT });
  404 |     } catch (err) {
  405 |       throw new Error(`${(err as Error).message}\n\n${await this.dumpRequestsModalState()}`);
  406 |     }
  407 |   }
  408 | 
  409 |   /**
  410 |    * `PAR-008` step 4's own confirmation: the request moved to the Denied tab.
  411 |    * Same `toPass` retry as `expectRequestApproved` above, and for the same
  412 |    * reason - see that method's header.
  413 |    */
  414 |   async expectRequestDenied(childName: string, requestedMinutes: number): Promise<void> {
  415 |     try {
  416 |       await expect(async () => {
  417 |         await screenTimeLocators.deniedTab(this.page).click();
  418 |         await this.widenEndDateFilter();
  419 |         const card = screenTimeLocators.requestCard(this.page, childName, requestedMinutes);
  420 |         await expect(
  421 |           screenTimeLocators.deniedStatusBadge(card),
  422 |           `the request for "${childName}" at ${requestedMinutes} minutes did not ` +
  423 |             'show up on the Denied tab after a successful denial.',
  424 |         ).toBeVisible({ timeout: 5_000 });
  425 |       }).toPass({ timeout: RENDER_TIMEOUT });
  426 |     } catch (err) {
  427 |       throw new Error(`${(err as Error).message}\n\n${await this.dumpRequestsModalState()}`);
  428 |     }
  429 |   }
```